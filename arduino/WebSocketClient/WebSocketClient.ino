...
void setup()
{
    ...
    if (!connect(server, "connect", port)) {
...
bool connect(const char *hostname, const char *path, uint16_t port){
    ...
    wifly.println(F("Sec-WebSocket-Key: bWlydGU=")); // base64 encoded something
    ...
    if (wifly.match(F("HTTP/1.1 101"), 30000)) {
        Serial.println("received handshake from server");
        wifly.println("handshake complete");
	wifly.flushRx(200);
	return true;
    }
